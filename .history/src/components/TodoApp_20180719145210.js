...
export default class TodoApp extends React.Component {
    ...
    filterHandler = (value) => {
        const nodeList = document.querySelector('.todo-list').childNodes;
        if(value !== 'all') {
            for(let i=0; i < nodeList.length; i++) { nodeList[i].setAttribute("style", "display: none;")}
        } else {
            for(let i=0; i < nodeList.length; i++) { nodeList[i].setAttribute("style", "display: block;")}
        }
    }
